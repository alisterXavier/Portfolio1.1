...
const Projects = () => {
  ...
  function toggleProjectModal(index: number, item: ProjectType) {
    setToggleProject({ index, item });
  }
  ...
  useGSAP(() => {
    ...
    const projectImageContainers = container.querySelectorAll('.project__item');
    const projectImages = container.querySelectorAll(
      '.project__item .project__item__inner__content'
    );

    gsap.set([projectImages, target.querySelector('.target__inner'), target], {
      width: projectImageContainers[0].clientWidth,
      height: projectImageContainers[0].clientHeight,
    });
    const boundX = container.clientWidth / 2;
    const boundY = container.clientHeight / 2;

    const drag = onDragFunctions(target, projectImageContainers, container);
    ...
    >
      ...
      <motion.div className="w-full h-full flex items-end relative z-[0]">
        <motion.div className="relative overflow-hidden w-[100%] h-[100%] bg-[var(--main)] flex items-center justify-center">
          <motion.div
            className="project__list absolute w-[300%] lg:h-[300%] md:h-[200%] h-[100%] flex flex-wrap justify-center items-center"
            ref={ref}
          >
            {projects.map((item, index) => (
              <motion.div
                key={index}
                className="project__item relative w-[20%] h-[20%] hover:cursor-pointer overflow-hidden flex items-center justify-center"
                layoutId={item.title + index}
                onClick={() => toggleProjectModal(index, item)}
              >
                {item.img.length && item.img.length > 0 ? (
                  <div className="project__item__inner overflow-hidden w-full h-full">
                    <figure className="relative project__item__inner__content">
                      <Image
                        src={item.img[0]}
                        alt={item.title}
                        fill
                  ...
                  <div className="relative w-full h-full project__item__inner overflow-hidden flex items-center justify-center">
                    <div className="absolute h-full w-full overflow-hidden border-3 border-default-accent project__item__inner__content flex items-center justify-center">
                      <p className="md:text-[100px] text-[var(--accent)]">
                        ¯\_(ツ)_/¯
                      </p>
                    </div>
                  </div>
