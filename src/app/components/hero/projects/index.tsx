...
export const Projects = (): JSX.Element => {
  ...
  const [positions, setPositions] =
    useState<{ element: Element; x: number; y: number }[]>();
  const ref = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    function getRandomPositions(arr: any, num: number): typeof positions {
      ...
      return Array.from(results) as typeof positions;
    }
    if (ref.current) {
      ...
      setPositions(getRandomPositions(calculatedPositions, 7));
    }
  }, [ref]);
  ...
  useGSAP(
    ...
    { dependencies: [state, positions], scope: ref, revertOnUpdate: true }
  );

  return (
    <CardWrapper
      className={`mouse_card hero_section col-span-full row-span-2 md:col-span-3 md:row-span-2 flex items-center justify-center`}
        ...
        >
          {projects.map((item, index) => {
            return item.img.length ? (
              <div
                className="project__item w-[300px] h-[150px] overflow-hidden"
                key={index}
              >
